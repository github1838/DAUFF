...
import logging
# # 设置一个文件处理器，指定文件路径和格式
# file_handler = logging.FileHandler("log/log026.txt")

import datetime
# 设置一个文件处理器，指定文件路径和格式
date = str(datetime.date.today())
# print("log/%s_log.txt" % date)
file_handler = logging.FileHandler("log/%s_log.txt" % date)
...
cfg.TRAIN_VIS_ITER_FERQ = 4680
cfg.TRAIN_VAL_ITER_FERQ = 1500
cfg.TRAIN_LOSS_WRITER_ITER_FERQ = 200
cfg.SEED_VALUE = -1
cfg.GPU = 0
cfg.RE_DEEP = True
cfg.DEEP = True
...
cfg.LOSS.KP_2D_W = 320.0
cfg.LOSS.KP_3D_W = 300.0
cfg.LOSS.SHAPE_W = 0.07
cfg.LOSS.POSE_W = 64.0
cfg.LOSS.VERT_W = 5.0
...
cfg.LOSS.DEPTH_WEIGHTS = 40.0

cfg.LOSS.DEPTH_BC_WEIGHTS = 0.7

cfg.MODEL = CN(new_allowed=True)

cfg.MODEL.PyMAF = CN(new_allowed=True)
cfg.FINAL_CHANNEL = 96  # 96

## switch
# cfg.TRAIN.VAL_LOOP = True
cfg.TRAIN.VAL_LOOP = False
