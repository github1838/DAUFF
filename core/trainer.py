...
from datasets.coco_occlusion import load_pascal_occluders
from models import hmr, pymaf_net, SMPL, SMPL_Family
...
# try:
#     from utils.renderer import  IUV_Renderer, DepthRender
# except:
#     print('fail to import IUV_Renderer.')
# try:
#     from utils.renderer import PyRenderer
# except:
#     print('fail to import PyRenderer.')
...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if self.options.regressor == 'hmr':
            ...
            # self.mesh_model = SMPL_Family(model_type='smpl', all_gender=False)
        ...
        self.criterion_shape = nn.SmoothL1Loss().to(self.device)
        ...
        self.criterion_depth_idx = nn.SmoothL1Loss().to(self.device)
        self.criterion_depth_bc = nn.BCEWithLogitsLoss().to(self.device)
        self.focal_length = constants.FOCAL_LENGTH

        self.depth_bins = self.options.depth_bins
        self.depth_mapsize = 56

        if self.options.pretrained_checkpoint is not None:
            self.load_pretrained(checkpoint_file=self.options.pretrained_checkpoint)
            logger.info('Loading pretrained checkpoint for stage two')
        ...
        occluders = None
        if self.options.use_synthetic_occlusion:
        ...
        if self.options.single_dataset:
            ...
            self.train_ds = BaseDataset(self.options, self.options.single_dataname, occluders=occluders, is_train=True)
        else:
            self.train_ds = MixedDataset(self.options, is_train=True)

        # n_samples_tr = len(self.train_ds)      # // 100  # // 20  # 计算要选择的数据点数量
        # indices_tr = random.sample(range(len(self.train_ds)), n_samples_tr) # 随机选择索引
        # self.train_ds_mini = Subset(self.train_ds, indices_tr)
        # logger.info('mini len of {}: {}'.format(self.train_ds.__name__(), self.train_ds_mini.__len__()))
        # self.train_ds = self.train_ds_mini

        self.valid_ds = BaseDataset(self.options, self.options.eval_dataset, is_train=False)
        # n_samples = len(self.valid_ds)    # // 5  # 计算要选择的数据点数量
        # indices = random.sample(range(len(self.valid_ds)), n_samples) # 随机选择索引
        # self.valid_ds_mini = Subset(self.valid_ds, indices)
        # logger.info('mini len of {}: {}'.format(self.valid_ds.__name__(), self.valid_ds_mini.__len__()))
        # self.valid_ds = self.valid_ds_mini 
        ...
        if cfg.MODEL.PyMAF.AUX_SUPV_ON:
            self.iuv_maker = IUV_Renderer(output_size=cfg.MODEL.PyMAF.DP_HEATMAP_SIZE, device=torch.device('cuda', cfg.GPU))
    ...
    def keypoint_pa_3d_loss(self, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d):
        """Compute 3D pa-keypoint loss for the examples that 3D keypoint annotations are available.
        The loss is weighted by the confidence.
        """
        pred_keypoints_3d = pred_keypoints_3d[:, 25:, :]
        conf = gt_keypoints_3d[:, :, -1].unsqueeze(-1).clone()
        ...
        if len(gt_keypoints_3d) > 0:
            ...
            pred_pelvis = (pred_keypoints_3d[:, 2,:] + pred_keypoints_3d[:, 3,:]) / 2
            pred_keypoints_3d = pred_keypoints_3d - pred_pelvis[:, None, :]
            S1_hat = compute_similarity_transform_batch(pred_keypoints_3d.detach().cpu().numpy(), gt_keypoints_3d.detach().cpu().numpy())
            S1_hat = torch.from_numpy(S1_hat).float().to(self.device)
            return (conf * self.criterion_keypoints(S1_hat, gt_keypoints_3d)).mean()
        else:
            return torch.FloatTensor(1).fill_(0.).to(self.device)
    ...
    def bin_idx_loss(self, depthimg_idx_pred, depth_gt,  has_depth=None):
        ...
        # background_mask = (depth_gt == min)
        foreground_mask = (depth_gt > -1)

        # back_depthimg_idx_pred = depthimg_idx_pred[background_mask]
        # back_depth_gt = depth_gt[background_mask]
        # back_loss = self.criterion_depth_idx(back_depthimg_idx_pred, back_depth_gt) 
        
        # ssim = SSIM(window_size = 11)
        # ssim_loss = 1 - ssim(depthimg_idx_pred.unsqueeze(1), depth_gt.unsqueeze(1))

        fore_depthimg_idx_pred = depthimg_idx_pred[foreground_mask]
        fore_depth_gt = depth_gt[foreground_mask]
        # fore_loss = F.smooth_l1_loss(fore_depthimg_idx_pred,fore_depth_gt, reduction='sum') / batch_size
        # fore_loss = F.huber_loss(reduction='sum') / batch_size
        fore_loss = self.criterion_depth_idx(fore_depthimg_idx_pred, fore_depth_gt) 

        loss_depth = fore_loss
        # loss_depth = (0.1 * back_loss + 0.9 * fore_loss)
        loss_depth *= cfg.LOSS.DEPTH_WEIGHTS
        return loss_depth
    
    def bin_bc_loss(self, depthimg_bc_pred, depth_gt,  has_depth=None):
        ...
        if has_depth is not None:
            if torch.sum(has_depth.float()) > 0:
                ...
                depth_gt = depth_gt[has_depth]
            else:
                return (torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device))
        if depthimg_bc_pred is None:
            loss_depth = None
        else:
            loss_depth = self.criterion_depth_bc(depthimg_bc_pred + 1e-10, depth_gt)
            loss_depth *= cfg.LOSS.DEPTH_BC_WEIGHTS
    ...
    def train(self, epoch):
        ...
        # Learning rate decaydecayRate = 0.96my_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=my_optim, gamma=decayRate)
        if self.decay_epochs_ind < len(cfg.SOLVER.EPOCHS) and epoch == cfg.SOLVER.EPOCHS[self.decay_epochs_ind]:
            lr = self.optimizer.param_groups[0]['lr']
            lr_new = lr * cfg.SOLVER.GAMMA
            logger.info('Decay the learning on epoch {} from {} to {}'.format(epoch, lr, lr_new))
        ...
        for step, batch in enumerate(self.train_data_loader, self.checkpoint_batch_idx):
            ...
            if self.options.single_dataset is False or self.options.eval_dataset == 'h36m-p2-mosh':
                if self.step_count % cfg.TRAIN_VAL_ITER_FERQ == 0 and self.options.rank == 0:
                    # if self.options.single_dataset and self.options.single_dataname == 'surreal':
                    # torch.cuda.empty_cache() 
                    # if self.options.eval_dataset == 'h36m-p2-mosh' and self.step_count < 150 * 1000:
                    #     continue
                    self.validate()

                    performance, m, p, v = self.evaluate()
                    ...
                    if is_best:
                        ...
                        self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch+1, 0, cfg.TRAIN.BATCH_SIZE,
                                                    self.step_count, is_best, save_by_step=True, m=m, p=p, v=v)
    ...
    def train_step(self, input_batch):
        ...
        opt_pose = input_batch['pose'] # SMPL pose parameters
        opt_betas = input_batch['betas'] # SMPL beta parameters
        ...
        # cliff bbox
        center = input_batch["center"]  # 裁剪区域的中心坐标
        scale = input_batch["scale"]
        # img_h = input_batch["img_h"]
        # img_w = input_batch["img_w"]
        img_focal = input_batch["img_focal"].to(self.device).float()
        img_center = input_batch['img_center']
        ...
        # opt_cam_t = estimate_translation(opt_joints, gt_keypoints_2d_orig, focal_length=img_focal, img_size=self.options.img_res)
        ...
        if self.options.regressor == 'pymaf_net' and cfg.MODEL.PyMAF.AUX_SUPV_ON:
            ...
            gt_camera[:, 0] = (2. * img_focal / self.options.img_res) / gt_cam_t_nr[:, 2] 
            iuv_image_gt = torch.zeros((batch_size, 3, cfg.MODEL.PyMAF.DP_HEATMAP_SIZE, cfg.MODEL.PyMAF.DP_HEATMAP_SIZE)).to(self.device)
            depth_gt = torch.zeros((batch_size, 1, self.depth_mapsize, self.depth_mapsize)).to(self.device)   
            if torch.sum(valid_fit.float()) > 0:
                iuv_image_gt[valid_fit], temp = self.iuv_maker.verts2iuvimg(opt_vertices[valid_fit], cam=gt_camera[valid_fit])  # [B, 3, 56, 56]
                depth_gt[valid_fit] = temp
            input_batch['iuv_image_gt'] = iuv_image_gt

            input_batch['depth_image_gt'] = depth_gt

            depth_bc_gt = torch.where(depth_gt<0,torch.zeros_like(depth_gt) , torch.ones_like(depth_gt))
            # 可视化
            # if torch.sum(valid_fit.float()) > 0:
            #     depth_bc_render[valid_fit] = depth_bc_gt[valid_fit]
            # input_batch['depth_bc_gt'] = depth_bc_render

            # max = torch.max(depth_image_render)
            # _max =53
            # _min = second_smallest 
            # # 计算张量中元素的直方图
            # hist = torch.histc(depth_image_render.flatten(), bins=_max-_min, min=_min,max=_max)

            # # 绘制直方图
            # plt.plot(hist.cpu().numpy())
            # plt.show()
            uvia_list = iuv_img2map(iuv_image_gt)
            depth_gt = depth_gt.squeeze(dim=1)
        ...
        if self.options.regressor == 'hmr':
            ...
            # 5
            preds_dict, _ = self.model(images, img_focal)      
        ...
        # 中间IUV损失
        # if cfg.RE_DEEP and self.options.single_dataset is False:
        #     dp_re_out = preds_dict['dp_re_out']
        #     for r_i in range(len(dp_re_out)):
        #         u_pred, v_pred, index_pred, ann_pred = dp_re_out[r_i]
        ...
        if cfg.DEEP:
            # 深度计算loss
            depth_out = preds_dict['depth_out']
            for i in range(len(depth_out)):
                # bins_gt: [B, K, H, W]
                # bin_idx_gt: [B, 1, H, W]

                r_i = i - len(depth_out)
                # 深度回归损失
                depth_idx_pred = depth_out[r_i]['predict_depth']
                depth_gt_i = F.interpolate(depth_gt, depth_idx_pred.shape[-1], mode='nearest')
                loss_bin_idx = self.bin_idx_loss(depth_idx_pred, depth_gt_i, valid_fit)
                loss_dict[f'loss_bin_idx{r_i}'] = loss_bin_idx
                
                # 二分类损失
                depth_bc_pred = depth_out[r_i]['predict_bc']
                depth_bc_gt_i = F.interpolate(depth_bc_gt, depth_bc_pred.shape[-1], mode='nearest')
                depth_bc_gt_i = depth_bc_gt_i.squeeze(dim=1)
                loss_bin_bc = self.bin_bc_loss(depth_bc_pred, depth_bc_gt_i, valid_fit)
                loss_dict[f'loss_bin_bc{r_i}'] = loss_bin_bc
        ...
        for l_i in range(len_loop):
            ...
            pred_cam_t = torch.stack([pred_camera[:,1],
                                    pred_camera[:,2],
                                    2*img_focal/(self.options.img_res * pred_camera[:,0] +1e-9)],dim=-1)
            ...
            if self.options.single_dataset is False or self.options.eval_dataset == 'h36m-p2-mosh':
            # multi datasets Compute 3D pa_keypoint loss
                loss_pa_keypoints_3d = self.keypoint_pa_3d_loss(pred_joints, gt_joints, has_pose_3d) * cfg.LOSS.KP_3D_W
                loss_dict['loss_pa_keypoints_3d_{}'.format(l_i)] = loss_pa_keypoints_3d
            
            # Per-vertex loss for the shape
            if cfg.LOSS.VERT_W > 0 and self.options.single_dataset is False:
        ...
        if self.step_count % cfg.TRAIN_LOSS_WRITER_ITER_FERQ == 0:
    ...
    def fit(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs, initial=self.epoch_count):
            ...
            if self.options.rank == 0:
                ...
                is_best = performance < self.best_performance
                if is_best:
                    logger.info('Best performance achived!')
                    self.best_performance = performance
                latest_checkpoint = self.saver.save_checkpoint(self.models_dict, self.optimizers_dict, epoch+1, 0, cfg.TRAIN.BATCH_SIZE, self.step_count, is_best)
    ...
    def validate(self):
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            # cliff bbox
            # center = target["center"].to(self.device)  # 裁剪区域的中心坐标
            # scale = target["scale"].to(self.device)
            # img_h = target["img_h"]
            # img_w = target["img_w"]
            img_focal = target["img_focal"].to(self.device).float()
            # img_center = target['img_center'].to(self.device)
            valid_fit = target['has_smpl'].to(torch.bool)

            # estimate focal length, and bbox
            # bbox_info = self.bbox_est(center, scale, img_focal, img_center)
            # bbox_info = bbox_info.to(self.device).float()
            
            inp = target['img'].to(self.device, non_blocking=True)
            batch_size = inp.shape[0]

            if 'h36m' in self.options.eval_dataset:
                # logger.info('h36m in self.options.eval_dataset')
                ...
                gt_joints = self.smpl_male(global_orient=gt_pose[:,:3], body_pose=gt_pose[:,3:], betas=gt_betas).joints
                gt_joints_female = self.smpl_female(global_orient=gt_pose[:,:3], body_pose=gt_pose[:,3:], betas=gt_betas).joints
            ...
            # De-normalize 2D keypoints from [-1,1] to pixel space
            gt_keypoints_2d_orig = target['keypoints'].clone()
            gt_keypoints_2d_orig[:, :, :-1] = 0.5 * self.options.img_res * (gt_keypoints_2d_orig[:, :, :-1] + 1)
    
            # Estimate camera translation given the model joints and 2D keypoints
            # by minimizing a weighted least squares loss
    
            target_cam_t = estimate_translation(gt_joints, gt_keypoints_2d_orig, focal_length=self.focal_length, img_size=self.options.img_res)
            target['target_cam_t'] = target_cam_t
    
            # Render Dense Correspondences
            if self.options.regressor == 'pymaf_net' and cfg.MODEL.PyMAF.AUX_SUPV_ON:
                gt_cam_t_nr = target_cam_t.detach().clone()
                gt_camera = torch.zeros(gt_cam_t_nr.shape).to(gt_cam_t_nr.device)
                gt_camera[:, 1:] = gt_cam_t_nr[:, :2]
                # gt_camera[:, 0] = (2. * self.focal_length / self.options.img_res) / gt_cam_t_nr[:, 2] 
                gt_camera[:, 0] = (2. * img_focal / self.options.img_res) / gt_cam_t_nr[:, 2] 
                ...
                depth_gt = torch.zeros((batch_size, 1, self.depth_mapsize, self.depth_mapsize)).to(self.device)   
                if torch.sum(valid_fit.float()) > 0:
                    iuv_image_gt[valid_fit], temp = self.iuv_maker.verts2iuvimg(gt_vertices[valid_fit], cam=gt_camera[valid_fit])  # [B, 3, 56, 56]
                    depth_gt[valid_fit] = temp
                
                target['iuv_image_gt'] = iuv_image_gt
    
                target['depth_image_gt'] = depth_gt
                
            
            J_regressor_batch = J_regressor[None, :].expand(inp.shape[0], -1, -1).contiguous().to(self.device, non_blocking=True)
                
            # 6
            pred_dict, _ = self.model(inp, img_focal, J_regressor=J_regressor_batch)     
    ...
    def evaluate(self):
        ...
        for loop_id in range(step):
            ...
            if self.options.single_dataset is True:
                log_str = f'Epoch {self.epoch_count}'
            else:
                log_str = f'Step {self.step_count}'
            log_str += ' '.join([f'{k.upper()}: {v:.4f},'for k,v in eval_dict.items()])
            logger.info(log_str)

            for k, v in eval_dict.items():
                if self.options.eval_dataset == 'h36m-p2-mosh':
                    self.summary_writer.add_scalar(f'eval_error/{k}_{loop_id}', v, global_step=self.step_count)
                else:
                    if self.options.single_dataset is True:
        ...
        # return mpjpe/79.7 + pa_mpjpe/49.0 + pve/94.4, mpjpe, pa_mpjpe, pve
        # return mpjpe/74.2 + pa_mpjpe/45.3 + pve/87, mpjpe, pa_mpjpe, pve
        # return mpjpe/78 + pa_mpjpe/47.1 + pve/91.3, mpjpe, pa_mpjpe, pve
        # return mpjpe/58 + pa_mpjpe/40, mpjpe, pa_mpjpe, pve
        return mpjpe/54.2 + pa_mpjpe/37.2, mpjpe, pa_mpjpe, pve
    ...
    def visualize(self, it, target, stage, preds, losses=None):
        ...
        if cfg.DEEP:
            depth_out = preds['depth_out'][-1] if cfg.MODEL.PyMAF.AUX_SUPV_ON else None    
        ...
        batch_size = images.shape[0]
        valid_fit = target['has_smpl'].to(torch.bool)
        # depth_image_gt_render = torch.zeros((batch_size, 3, self.depth_mapsize, self.depth_mapsize)).to(self.device)
        # depth_bc_gt_render = torch.zeros((batch_size, 3, self.depth_mapsize, self.depth_mapsize)).to(self.device)
        if torch.sum(valid_fit.float()) > 0:
            depth_image_gt_render = target['depth_image_gt'].clamp(min = -0.0025).expand(-1,3,-1,-1)
        else:
            depth_image_gt_render = torch.zeros((batch_size, 3, self.depth_mapsize, self.depth_mapsize)).to(self.device)
        ...
        for b in range(vis_n):
            ...
            if cfg.MODEL.PyMAF.AUX_SUPV_ON:
                if stage == 'train':    
                    depth_image_gt = depth_image_gt_render[b].detach().cpu().numpy()
                ...
                if cfg.DEEP:
                    depth = depth_out['predict_depth']# b,1,h,w
                    depth_bc = depth_out['predict_bc']# b,1,h,w
                    depth_bc = torch.round(torch.sigmoid(depth_bc))

                    depth_image_render = torch.zeros((depth.shape[0], 3, self.depth_mapsize, self.depth_mapsize)).to(self.device)
                    # depth_bc_render = torch.zeros((depth.shape[0], 3, self.depth_mapsize, self.depth_mapsize)).to(self.device)
                    depth_temp = torch.zeros((depth.shape[0], self.depth_mapsize, self.depth_mapsize)).to(self.device)
                    
                    depth_batch = depth[b].clamp(min = -0.0025)
                    depth_batch = (depth_batch + torch.ones_like(depth_batch) * 0.0025) * 400 / 401 #  + (self.depth_bins - 1) / 2 # -10 0-40

                    # 可视化只取二分类预测中前景部分
                    depth_temp[b, depth_bc[b] != 0] = depth_batch[depth_bc[b] != 0]

                    # depth = torch.stack(depth_list, dim=0)# b,h,w
                    depth_image_render = depth_temp.unsqueeze(1).expand(-1,3,-1,-1)
                    depth_image_pred = depth_image_render[b].detach().cpu().numpy()
                ...
                # 数据分布
                # depth_num_pred = depth[b, depth_bc[b] != 0].detach().cpu().numpy().flatten()
                
                # plt.figure(figsize=(2, 2))
                # plt.scatter(range(len(depth_num_pred)), depth_num_pred, s=1)
                # plt.xlabel('Index')
                # plt.ylabel('Value')
                # fig = plt.gcf()
                # canvas = fig.canvas
                ...
                # image_array = np.frombuffer(canvas.tostring_rgb(), dtype='uint8').reshape(height, width, 3)
                # depth_num_pred_resized = resize(image_array, (img_vis.shape[0], img_vis.shape[1]),
                #                                             preserve_range=True, anti_aliasing=True)
                # render_imgs.append(depth_num_pred_resized.astype(np.uint8))
                # plt.close()
            # 二分类可视化
            # if cfg.MODEL.PyMAF.AUX_SUPV_ON:
            #     if stage == 'train':                       
            #         depth_bc_gt = target['depth_bc_gt'][b].detach().cpu().numpy()
            #         depth_bc_gt = np.transpose(depth_bc_gt, (1, 2, 0)) * 255

            #         depth_bc_gt_resized = resize(depth_bc_gt, (img_vis.shape[0], img_vis.shape[1]),
            #                                                 preserve_range=True, anti_aliasing=True)
            #         render_imgs.append(depth_bc_gt_resized.astype(np.uint8))

            #     depth_bc_render = depth_bc.unsqueeze(1).expand(-1,3,-1,-1)
            #     depth_bc_pred = depth_bc_render[b].detach().cpu().numpy()
            #     depth_bc_pred = np.transpose(depth_bc_pred, (1, 2, 0)) * 255
            #     depth_bc_pred_resized = resize(depth_bc_pred, (img_vis.shape[0], img_vis.shape[1]),
            #                                                 preserve_range=True, anti_aliasing=True)
            #     render_imgs.append(depth_bc_pred_resized.astype(np.uint8))
