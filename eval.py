...
parser.add_argument('--checkpoint', default='/home/wz/work_dir/PyMAFtest/logs/pymaf_hr48_mix/pymaf_hr48_mix_as_lp3_mlp256-128-64-5_Oct19-20-48-53-Urr/checkpoints/model_best_72.269_43.288_83.506.pt', help='Path to network checkpoint')
...
def run_evaluation(model, dataset):
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        # cliff bbox
        center = batch["center"].to(device)  # 裁剪区域的中心坐标
        scale = batch["scale"].to(device)
        img_focal = batch["img_focal"].to(device).float()
        img_center = batch['img_center'].to(device)
        bbox_info = bbox_est(center, scale, img_focal, img_center)
        bbox_info = bbox_info.to(torch.device('cuda')).float()
        ...
        with torch.no_grad():
            if args.regressor == 'hmr':
                ...
                preds_dict, _ = model(images, img_focal)
