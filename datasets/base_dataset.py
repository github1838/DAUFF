...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False, use_augmentation=True, is_train=True, occluders=None):
        super().__init__()
        ...
        try:
            ...
            if 'has_smpl' in self.data:
                ...
                logger.info(self.dataset + ' has_smpl')
            else:
        ...
        self.occluders = None
        if self.is_train and self.options.use_synthetic_occlusion:
            logger.info('Using mixed dataset occluders')
    ...
    def rgb_processing(self, rgb_img, center, scale, rot, flip, pn, kp2d, has_smpl):
        """Process rgb image and do augmentation."""
        rgb_img = crop(rgb_img, center, scale, 
                      [constants.IMG_RES, constants.IMG_RES], rot=rot)
        
        if self.occluders is not None and has_smpl:
            if self.options.occ_aug_dataset == 'coco':
                rgb_img = occlude_with_coco_objects(rgb_img, kp2d=kp2d.copy(),
                                                    occluders=self.occluders, img_size=constants.IMG_RES)
    ...
    def __getitem__(self, index):
        ...
        item["img_h"] = img_h
        item["img_w"] = img_w
        item["focal_length"] = (img_w * img_w + img_h * img_h) ** 0.5
        item['dataset'] = self.dataset
        # b = scale*200/item["focal_length"]
        # if b > 0.75 or b < 0.15:
        #     sc = 1
        ...
        kps2d = self.j2d_processing(keypoints, center, sc*scale, rot, flip, kp_is_smpl)
        # Process image
        # img = self.rgb_processing(img, center, sc*scale, rot, flip, pn)
        # Process image syn occ
        img = self.rgb_processing(img, center, sc*scale, rot, flip, pn, kp2d=kps2d, has_smpl=self.has_smpl[index])
