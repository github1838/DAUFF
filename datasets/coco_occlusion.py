...
from core.constants import get_spin_joint_names
...
def load_pascal_occluders(pascal_voc_root_path):
    if os.path.isfile(path_config.PASCAL_OCCLUDERS_FILE):
        return joblib.load(path_config.PASCAL_OCCLUDERS_FILE)
    else:
        occluders = []
        ...
        joblib.dump(occluders, '/home/wz/work_dir/PyMAFtest/data/occlusion_augmentation/pascal_occluders.pkl')
        return occluders


def load_coco_occluders(path=None):
    occluders = joblib.load(path_config.COCO_OCCLUDERS_FILE)
