...
def depth_img2map(depth_images):
    depth_bc_images = torch.where(depth_images<0,torch.zeros_like(depth_images) , torch.ones_like(depth_images))
    return depth_images, depth_bc_images
    # return depth_images, torch.zeros_like(depth_bc_images)
    # return torch.zeros_like(depth_images), depth_bc_images
