...
class PyRenderer:
    def __init__(self, resolution=(224, 224), orig_img=False, wireframe=False, scale_ratio=1., vis_ratio=1.):
        ...
        self.device = cfg.CUDA_VISIBLE_DEVICES
    ...
    def __call__(self, verts, faces=None, img=np.zeros((224, 224, 3)), cam=np.array([1, 0, 0]),
                 ...
                 crop_img=True, mesh_type='smpl', scale_ratio=1., rgba_mode=False, renderpoints=False):

        if faces is None:
            faces = self.faces[mesh_type]
        # if renderpoints is True:
        #     mesh = pyrender.Mesh.from_points(verts)
        ...
        if renderpoints is True:
            Rx = trimesh.transformations.rotation_matrix(math.radians(180), [1, 0, 0])
            hom_verts = np.column_stack((verts, np.ones(len(verts))))
            verts = np.dot(Rx, hom_verts.T).T[:, :3]
...
class IUV_Renderer(object):
    def __init__(self, focal_length=5000., orig_size=224, output_size=56, mode='iuv', device=torch.device('cuda', cfg.GPU), mesh_type='smpl'):
    ...
    def verts2iuvimg(self, verts, cam, iwp_mode=True):
        ...
        iuv_image, depth = self.renderer(mesh, cameras=cameras)
        ...
        for i in range(depth.shape[0]):
            ...
            if depth_batch[foreground_mask].numel()!=0:
                ...
                if depth_max - depth_min == 0:
                    normalized_batch = depth_batch - depth_max 
                else:
                    # depth_batch = depth_batch - depth_min
                    # depth_batch = depth_batch / (depth_max - depth_min)
                    normalized_batch = (depth_batch - depth_min) / (depth_max - depth_min)

                normalized_batch = torch.where(normalized_batch<0, torch.ones_like(normalized_batch) * -1, normalized_batch)
                depth_list.append(normalized_batch)
            else:
                depth_list.append(depth_batch)

        return iuv_image, torch.stack(depth_list, dim=0)


# class DepthRender:
#     def __init__(self, focal_length=5000., orig_size=224, output_size=56, device=torch.device('cuda')):
...
#         vertices = verts
#         mesh = Meshes(vertices, faces.to(verts.device).expand(batch_size, -1, -1))

#         cameras = PerspectiveCameras(device=verts.device, R=R, T=t, K=K, in_ndc=False, image_size=[(self.orig_size, self.orig_size)])

#         _, depth = self.renderer(mesh, cameras=cameras).permute(0, 3, 1, 2)
