...
class CheckpointSaver():
    ...
    def save_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size,
                        total_step_count, is_best=False, save_by_step=False, interval=2, m=None, p=None, v=None):
        """Save checkpoint."""
        timestamp = datetime.datetime.now()
        if self.overwrite:
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'model_latest_'+ str(epoch) + '.pt'))
        elif save_by_step:
        ...
        if save_by_step:
            if is_best: # save the best
                checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'model_best'+'_'+
                                                                   str(round(m,3))+'_'+
                                                                   str(round(p,3))+'_'+
                                                                   str(round(v,3))+'.pt'))
                ...
                # logger.info('Epoch:' + str(epoch))
                # torch.save(checkpoint, checkpoint_filename)
                # logger.info('Saved checkpoint file [' + checkpoint_filename + ']')
            return checkpoint_filename
        else:
            if checkpoint_filename is not None:
                torch.save(checkpoint, checkpoint_filename)
                logger.info('Saving checkpoint file [' + checkpoint_filename + ']')
            if is_best: # save the best
